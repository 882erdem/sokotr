...
public class GameUtil {
    ...
    public static void sendGameEmbed(MessageChannel channel, String level, String game, User user) {
        ...
        embed.addField("Harket için (``up``, ``down``, ``left``, ``right``/``wasd``), ``r`` reset için ``mr`` "
                               + "haritayı yeniden oluşturur", "", false);
        embed.addField("Oyuncu", user.getAsMention(), false);
    ...
    public static void sendWinEmbed(Guild guild, Message message, String level) {
        ...
        embed.setDescription(
                "Type ``" + Bot.getPrefix(guild) + "Sonraki`` Seviyeye devam etmek " + level + " veya ``" + Bot
                        .getPrefix(guild) + "stop`` = çıkış ");
        embed.setFooter("Devam etmek için herhangi bir reaksiyona da basabilirsiniz.");
        message.editMessage(embed.build()).queue();
    }
