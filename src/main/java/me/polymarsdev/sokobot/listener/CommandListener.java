...
public class CommandListener extends ListenerAdapter {
    ...
    private void sendInvalidPermissionsMessage(User user, TextChannel channel) {
        if (channel.canTalk()) {
            ...
            channel.sendMessage(user.getAsMention() + ", Düzgün çalışmak için yeterli izne sahip değilim. "
                                        + "elbette aşağıdaki izinlere sahibim: " + requiredPermissionsDisplay
                                        + "\nEğer bunun olduğunu düşünüyorsan "
                                        + "bir hata, lütfen bir sunucu yöneticisine başvurun.").queue();
        }
    }
}